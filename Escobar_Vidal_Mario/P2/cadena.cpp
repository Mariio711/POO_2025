...
{
    // si el stream ya tiene error, no hacer nada
    if (!is)
    {
        return is;
    }
    ...
    char cad[33];
    cad[0] = *Cadena::vacia; // inicializo buffer
    is >> std::ws; //para consumir espacios iniciales

    if (is.peek() == EOF)
    {
        // entrada vacia despues de espacios
        c = Cadena(cad);
    ...
    is.width(33);
    if (is >> cad) // solo si lee algo valido
    {
        c = Cadena(cad);
    }
    return is;
